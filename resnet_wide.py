...
'''
class StdConv2d(nn.Conv2d):  # Weight standardization
...
class WideBasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='B'):
        ...
        self.n1 = norm_fn(num_channels=in_planes)
        ...
        if stride != 1 or in_planes != self.expansion * planes:
            if option == 'A':
                ...
                self.shortcut = nn.Sequential(
                     nn.ReLU(),
                     norm_fn(num_channels=in_planes),
                     StdConv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False),
                )

    def forward(self, x):
        '''
        out = self.conv1(self.n1(active(x)))
        out = self.conv2(self.n2(active(out)))
        out += self.shortcut(x)
        '''
        NLA = F.elu
        out = self.n1(NLA(self.conv1(x)))
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, width=4, num_classes=10):
        ...
        self.gn1 = norm_fn(num_channels=self.in_planes)
        self.layer1 = self._make_layer(block, 16, num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, 32, num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, 64, num_blocks[2], stride=2)
        self.final_norm = norm_fn(num_channels=self.in_planes)
    ...
    def forward(self, x):
        ...
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.final_norm(active(out))
        out = F.avg_pool2d(out, out.size()[3])
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        '''
        # out = self.n1(active(self.conv1(x)))
        out = self.gn1(F.elu(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
...
y = model(x)
b = []
for model_ in model.named_parameters():
    b.append(model_[0])
